   ...
   <h1 class="mb-2 mt-2">My Polls</h1>
    ...
    <div *ngIf="polls" class="pb-5">
        ...
        <mat-card *ngFor="let poll of polls" [routerLink]="[poll.id]" class="mb-1 linked-card">
          ...
          <mat-card-subtitle>{{poll.is_published ? 'Published' : 'Draft' }} - {{false ? 'Open' : 'Closed' }} – {{poll.vote_count ? poll.vote_count + ' votes' : 'No Votes'}} {{ poll.is_promoted ? ' – Promoted Poll' : '' }}</mat-card-subtitle>
        </mat-card>
